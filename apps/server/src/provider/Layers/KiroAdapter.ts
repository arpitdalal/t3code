...
const resolveNotificationTurnId = (ctx: KiroSessionContext): TurnId | undefined => ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: KiroSessionContext): TurnId | undefined => ctx.activeTurnId;

const resolveSessionCallbackTurnId = (
  sessions: ReadonlyMap<ThreadId, KiroSessionContext>,
  threadId: ThreadId,
): TurnId | undefined => {
...
export function kiroPromptSettlementBelongsToContext(input: {
  ...
  return Effect.gen(function* () {
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = kiroPromptSettlementBelongsToContext({
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to write native Kiro notification log.", {
            cause,
            threadId,
            method,
          }),
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolveKiroAcpBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const displayModel = currentModelId
                ? resolveKiroAcpBaseModelId(currentModelId)
                : undefined;
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "Kiro prompt was interrupted during preparation.",
                });
              }
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "Kiro prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "Kiro session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "Kiro session changed before the turn completed.",
                });
              }
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "Kiro prompt request failed.",
                }),
              );
    ...
    const rollbackThread: KiroAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "Kiro ACP sessions do not support provider-side rollback yet.",
        });
      });
